...
class PantsScalaDataService extends ProjectDataService[ScalaModelData, Library] {
  ...
  override def importData(toImport: util.Collection[_ <: DataNode[ScalaModelData]],
                          data: ProjectData,
                          project: Project,
  ...
  override def computeOrphanData(toImport: util.Collection[_ <: DataNode[ScalaModelData]],
                                 ...
                                 project: Project,
                                 modelsProvider: IdeModifiableModelsProvider): Computable[util.Collection[Library]] =
    () => Collections.emptyList()

  override def removeData(computable: Computable[_ <: util.Collection[_ <: Library]],
                          collection: util.Collection[_ <: DataNode[ScalaModelData]],
                          data: ProjectData,
                          project: Project,
                          provider: IdeModifiableModelsProvider): Unit = {
  }
}
